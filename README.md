...
- Selecting an MSI through file import, folder import or an alternative entrypoint automatically fills its ProductCode from the local MSI database. The field remains editable; unreadable MSI metadata clears an old code and prompts for manual entry. Registry uninstall discovery offers registered normal/quiet commands for review; commands with spaces in the executable path must quote that path. MSI ProductCode takes precedence when uninstalling.
