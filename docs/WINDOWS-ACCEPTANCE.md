...
Known boundaries: there is no reboot resume, vendor installer download, execution of proprietary EXE wrappers for extraction, or post-reboot Syncro registration validation. Analyze Installer scans at most the first 16 MiB for signatures and enumerates already imported MSI candidates; it can carve up to four uncompressed compound-file candidates from EXEs up to 512 MiB and offers only those validated by a readable MSI ProductCode. Compressed/encrypted vendor payloads remain manual. It never runs an unknown wrapper to extract payloads. Installer stdout/stderr is intentionally not persisted. The app retains imported media when a definition is deleted. External installers and Windows necessarily write their own application/system data to the target PC; the portability guarantee covers CIT Deploy's configuration, media, logs, backups and runtime.

Optional Syncro: create and save a clinic without Syncro, create its profile and deploy. Preflight must allow the absent mapping, the Syncro step must show Skipped, and domain joining must remain available. Import Syncro and verify it installs; remove its mapping and verify subsequent deployments skip it. A configured path whose file is missing must still fail media preflight.

Optional domain: save a clinic with a blank domain, deploy applications without domain connectivity, and verify no domain lookup, credential prompt, join or rename occurs. The domain step must be Skipped and the existing computer name kept. No reboot is required solely for the skipped domain step; installer reboot requirements still apply. Repeat with a valid domain and verify the normal join workflow remains available.
